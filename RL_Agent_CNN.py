...
class DrawAgent:
    ...
    def train(self, initstate, agentStartState, **params):
        ...
        maxstep = params.pop('maxstep', 784)
        ...
        for j in range(1, maxiter):
            ...
            for _step in range(maxstep):
                ...
                #my_state = np.expand_dims(f_s, axis=0)
                my_state = np.reshape(f_s, (28,28,1))
                ...
                self.experience.append([s, f_s ,a, r, s1, f_s1])
                ...
                if _step > self.update_nn:
                    ...
                    minibatch = random.sample(self.experience, 64)
                    ...
                    _tmp_s1 = np.vstack([np.reshape(fs1, (28,28,1))] for fs1 in _fs1)
                    _tmp_s = np.vstack([np.reshape(fs, (28,28,1))] for fs in _fs)                    
                    #_tmp_s1 = np.vstack(_fs1)
                    #_tmp_s = np.vstack(_fs)
                ...
                # Now we want to reduce the loss for the NN, so that it will converge to a good weights
                    ...
                    for i in range(len(minibatch)):
                        if self.benv.checkGoal(_fs[i]):
                            _tmp_y = np.zeros(self.n_a)
                            _tmp_index = np.argmax(tmp_a1[i])
                            _tmp_y[_tmp_index] = _r[i] + self.gamma * tmp_a1[i][_tmp_index]
                            _y.append(_tmp_y)
                        else:
                            _tmp_index = np.argmax(tmp_a1[i])
                            _tmp_y = np.zeros(self.n_a)
    ...
    def test(self, initstate, start, maxstep=1000):
        ...
        _tmp_s = np.reshape(np.copy(self.benv.state), (784,))
        my_state = np.reshape(_tmp_s, (28,28,1))
        my_state = np.expand_dims(my_state, axis=0)
        # selection an action
